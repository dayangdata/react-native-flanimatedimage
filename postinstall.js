...
try {
  ...
  console.warn('You should run react-native-flanimatedimage from root of your project')
}
...
if (process.platform === 'darwin' && shouldUseCarthage()) {
  ...
  if (carthageVersionProcessResult.status != 0) {
    ...
    errorExitProcess(
      'carthage is required to compile frameworks for iOS backend. You can install it with brew: "brew install carthage". After installation go to ./node_modules/react-native-flanimatedimage and run "./postinstall.js" or reinstall node module'
    )
  }
  ...
  try {
    ...
    errorExitProcess(`${rnFlanimatedImageDirectory} directory not found. Cannot proceed with building the library.`)
  }
